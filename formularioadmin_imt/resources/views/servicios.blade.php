<x-app-layout>
                ...
                <div class="p-6 text-gray-900 table-container" id="tableContainer">
                    ...
                    <div class="w-full min-w-0" 
                         x-data="{
                            ...
                            searchQuery: '',
                            totalRows: {{ $servicios->count() }},
                            visibleRows: {{ $servicios->count() }},
                            ...
                            get filteredServicios() {
                                if (!this.searchQuery) {
                                    ...
                                    return [...this.servicios].sort((a, b) => b.id - a.id);
                                }
                                
                                const query = this.normalize(this.searchQuery);
                                const filtered = this.servicios.filter(servicio => {
                                    return this.normalize(servicio.nombre).includes(query)
                                        || this.normalize(servicio.id.toString()).includes(query)
                                });
                            ...
                            openEditModal(servicio) {
                                this.editData = {
                                    ...
                                    coordinacion_predeterminada_id: servicio.coordinacion_predeterminada_id || '',
                                };
                                this.showEditModal = true;
                            },
                            
                            openDeleteModal(servicio) {
                                this.deleteData = {
                                    id: servicio.id,
                                    nombre: servicio.nombre
                                };
                            ...
                            getCoordinacionNombre(coordinacionId) {
    ...
    <tr>
        <th>ID</th>
        <th>Nombre</th>
        <th>Coordinación Predeterminada</th>
        ...
        <th style="text-align: center;">Acciones</th>
                                    ...
                                    <tbody>
                                        <template x-for="servicio in filteredServicios" :key="servicio.id">
                                            <tr>
                                                <td x-text="servicio.id"></td>
                                                <td x-text="servicio.nombre"></td>
                                                <td x-text="getCoordinacionNombre(servicio.coordinacion_predeterminada_id)"></td>
                                                <td x-text="formatDate(servicio.fecha_creacion)"></td>
                                                <td style="text-align: center;">
                                                    <div style="display: flex; align-items: center; justify-content: center; gap: 0.5rem;">
                                                        ...
                                                        <button 
                                                            @click="openDeleteModal(servicio)"
                                                            class="btn-delete"
                                                        >
                                                            Eliminar
                                                        </button>
                        ...
                        <div x-show="showCreateModal" x-cloak class="modal-overlay" @click.self="showCreateModal = false">
                            <div class="modal-content">
                                <div class="modal-header">
                                    <h3 class="text-lg font-semibold">Crear Nuevo Servicio</h3>
                        ...
                        <div x-show="showEditModal" x-cloak class="modal-overlay" @click.self="showEditModal = false">
                            <div class="modal-content">
                                <div class="modal-header">
                                    <h3 class="text-lg font-semibold">Editar Servicio #<span x-text="editData.id"></span></h3>
                                ...
                                <form method="POST" :action="`/servicios/${editData.id}`" class="modal-body">
                                    @csrf
                                    @method('PUT')
                                    <div class="form-group">
                                        <label class="form-label">Nombre *</label>
                                    ...
                                    <div class="form-group">
                                        <label class="form-label">Coordinación Predeterminada</label>
                                        <select name="coordinacion_predeterminada_id" x-model="editData.coordinacion_predeterminada_id" class="form-input">
                                            <option value="">Seleccionar coordinación</option>
                                            @if(isset($coordinaciones))
                                                @foreach($coordinaciones as $coordinacion)
                                                    <option value="{{ $coordinacion->id }}">{{ $coordinacion->nombre }}</option>
                            ...
                            <div class="modal-content">
                                ...
                                <div class="modal-body">
                                    <p class="text-gray-600 mb-6">
                                        ¿Estás seguro de que deseas eliminar el servicio "<span x-text="deleteData.nombre" class="font-semibold"></span>"?
                                    </p>
                                    ...
                                    <div style="display: flex; justify-content: flex-end; gap: 12px;">
                                        ...
                                        <form method="POST" :action="`/servicios/${deleteData.id}`" style="display: inline;">
                                            @csrf
                                            @method('DELETE')
